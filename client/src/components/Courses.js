...
export default class Courses extends Component {
  ...
  render() {
    // why do I have to do this? why can't I use map or forEach? everytime I do it says its not a function
    const courses = []
    const coursesState = this.state.courses
    for (let course of coursesState) {
      courses.push(<CourseButton title={course.title} id={course.id} key={course.id} />)
    }
